...
export function ClientAppHome({ services }: { services: AppService[] }) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f8f5] text-[#191919]">
      <header className="sticky top-0 z-40 border-b border-zinc-200 bg-[#f7f8f5] px-4 py-3">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <div>
            <p className="text-base font-black leading-none text-[#111]">PinoyBoosting</p>
            <p className="mt-1 text-xs font-semibold text-zinc-500">Simple mobile app</p>
          </div>
          <button
            type="button"
            onClick={handleUpdate}
            disabled={isUpdating}
            className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-full border border-zinc-200 bg-white px-3 text-xs font-bold text-zinc-800 shadow-sm disabled:cursor-wait disabled:opacity-70"
          >
      ...
      <div className="mx-auto w-full max-w-3xl px-4 pb-28 pt-5">
        <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-sm font-medium leading-6 text-zinc-600">
            Pick what you need, add your link, then track the order. No extra website effects inside the APK.
          </p>

          <label className="mt-5 flex h-12 items-center gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4">
        ...
        <section className="mt-5">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-sm font-black text-zinc-900">Most used</h2>
            <button
              type="button"
              onClick={() => openCatalog("")}
            ...
            {SHORTCUTS.map((shortcut) => {
              ...
              return (
                <button
                  ...
                  className="flex min-h-28 items-start gap-3 rounded-3xl border border-zinc-200 bg-white p-4 text-left shadow-sm transition active:scale-[0.99]"
                >
                  <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border ${shortcut.tone}`}>
                    <Icon size={20} />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block text-sm font-black text-zinc-950">{shortcut.title}</span>
            ...
            {featuredServices.length > 0 ? (
              featuredServices.map((service, index) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-4 py-4 text-left transition active:bg-zinc-50 ${
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-black text-zinc-950">{service.title}</span>
                    <span className="mt-1 block line-clamp-2 text-xs font-medium leading-5 text-zinc-500">
              ...
              <div className="p-5 text-sm font-medium leading-6 text-zinc-500">
