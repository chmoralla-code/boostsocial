  ...
  return (
    <div className={compact ? "space-y-5" : "space-y-6"}>
      <div className="space-y-3">
        ...
        <div>
          <h2 className="text-2xl font-black tracking-tight text-white sm:text-3xl">Earn by sharing your link</h2>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        {["Copy your link", "Share it online", "Earn commission"].map((step, index) => (
          <div key={step} className="rounded-xl border border-slate-800 bg-[#121212] p-4">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Step {index + 1}</span>
            <p className="mt-1 text-sm font-extrabold text-white">{step}</p>
          </div>
        ))}
      </div>

      <div className="rounded-xl border border-slate-800 bg-[#121212] p-4 sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="min-w-0 flex-1">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Your referral link</span>
            <div className="mt-2 truncate rounded-lg border border-slate-800 bg-[#181818] px-3 py-2 font-mono text-xs font-bold text-slate-300">
              {inviteLink}
            </div>
          </div>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:w-72">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-lg border border-slate-700 bg-[#202020] px-4 py-3 text-xs font-black uppercase tracking-wider text-white transition hover:bg-[#2a2a2a]"
            >
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
        <div className="rounded-xl border border-[#1DB954]/25 bg-[#1DB954]/10 p-4">
          ...
          <p className="mt-1 text-lg font-black text-white">{activeTier.name}</p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-[#121212] p-4">
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Order Rate</span>
          <p className="mt-1 text-lg font-black text-white">{formatReferralRate(activeTier.orderRate)}</p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-[#121212] p-4">
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Top-Up Rate</span>
          <p className="mt-1 text-lg font-black text-white">{formatReferralRate(activeTier.topupRate)}</p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-[#121212] p-4">
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Total Earned</span>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_280px]">
        ...
        <div className="rounded-xl border border-slate-800 bg-[#121212] p-4 sm:p-5">
          ...
          <button
            ...
            className="mt-4 w-full rounded-lg bg-[#1DB954] px-4 py-3 text-xs font-black uppercase tracking-wider text-black transition hover:bg-[#1ed760] disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-slate-500"
          >
      ...
      <div className="rounded-xl border border-slate-800 bg-[#121212] p-4 sm:p-5">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">Recent referral activity</h3>
          <span className="text-[10px] font-bold text-slate-500">{summary.inviteCount} referrals</span>
        </div>

        {loading ? (
          <div className="flex flex-col items-center justify-center gap-2 py-8">
            <Loader2 className="animate-spin text-[#1DB954]" size={20} />
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Loading dashboard...</span>
          </div>
        ) : summary.transactions.length === 0 ? (
          <div className="rounded-lg border border-slate-850 bg-[#181818] p-5 text-center text-xs font-semibold text-slate-500">
            ...
            {summary.transactions.map((tx) => (
              <div key={tx.id} className="flex items-center justify-between gap-4 py-3 text-xs">
                <div className="min-w-0">
                  <p className="font-bold text-white">{tx.refereeLabel}</p>
                  <p className="truncate text-[10px] font-semibold text-slate-500">{tx.description}</p>
                </div>
                <div className="shrink-0 text-right">
                  <p className="font-black text-[#1DB954]">+{money(tx.amount)}</p>
                  <p className="flex items-center justify-end gap-1 text-[9px] text-slate-500">
                    <Calendar size={9} />
...
export function ReferralsModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#090909]/80 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative flex max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-800/80 bg-[#181818] shadow-2xl animate-in zoom-in-95 duration-200">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 z-10 rounded-lg p-1 text-slate-400 transition-colors hover:bg-slate-800/50 hover:text-white"
